...
city.addEventListener("change", () => {
    ...
    .then(data => {
      ...
      .insertAdjacentHTML("beforeend", `<p>${new Date().toLocaleDateString('ru-RU',{month: 'long'})}, ${new Date().getDate()}</p>`);
    ...
    data.daily.forEach((element) => {
      document.querySelector(".scrool").insertAdjacentHTML(
       ...
       `
        ...
        date.insertAdjacentHTML('beforeend', `<p>Сегодня, ${new Date().getDate()} ${new Date().toLocaleDateString('ru-RU',{month: 'long'})}</p>`)
